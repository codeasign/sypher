...
import { isUnderQuota, recordEmailSend, type EmailProviderName } from './emailQuota';
...
export interface SendEmailOptions {
  // Test-only: forces these providers to fail without making a real vendor
  // call, so the admin test endpoint can verify fallback without needing to
  // actually exhaust or break a live account. Never set outside tests.
...
export async function sendEmailWithRotation(params: SendEmailParams, options?: SendEmailOptions): Promise<SendEmailResult> {
  ...
  for (const provider of PROVIDERS) {
    ...
    if (options?.simulateFailureFor?.includes(provider.name)) {
      result = { success: false, provider: provider.name, error: 'simulated failure (test)' };
    } else {
