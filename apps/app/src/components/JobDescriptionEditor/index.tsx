...
// aborts the entire chunk's evaluation and leaves
// JobDescriptionEditorInner's dynamic import() promise permanently
// unresolved.
//
...
// BlogPostEditor/index.tsx -- same @mdxeditor/editor bundling issue,
// applies per-entrypoint.
...
function JobDescriptionEditorLoader(props: JobDescriptionEditorProps): React.JSX.Element {
  const [Inner, setInner] = useState<ComponentType<JobDescriptionEditorProps> | null>(null);

  useEffect(() => {
    import('./JobDescriptionEditorInner').then((mod) => setInner(() => mod.default));
  }, []);
...
export default function JobDescriptionEditor(props: JobDescriptionEditorProps): React.JSX.Element {
  return <JobDescriptionEditorLoader {...props} />;
}
