...
```sql practice
-- hint: `DISTINCT length ... ORDER BY length DESC LIMIT 1 OFFSET 2`, wrapped in a scalar subquery.
SELECT (
...
```sql practice
-- hint: `DENSE_RANK() OVER (ORDER BY length DESC)` in a CTE, keep `rk = 2`.
WITH ranked AS (
...
```sql practice
-- hint: Total per customer in a CTE, then DENSE_RANK on the totals.
