...
```sql run
SELECT title, length, length / 60
FROM film;
```

The last column has an ugly name, because MySQL just used the expression text. You will fix that in the next page with an *alias*.
...
MySQL tells you which column it could not find. Read the message: most SQL mistakes are explained in it.

### SELECT does not change anything

`SELECT` only reads. Nothing you do with it can damage the data. Experiment freely.
...
Yes, for expressions and functions, such as `SELECT NOW();`.
