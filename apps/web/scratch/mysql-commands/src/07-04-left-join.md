...
```sql run
SELECT COUNT(*) AS filter_in_where
FROM film AS f
LEFT JOIN inventory AS i ON i.film_id = f.film_id
WHERE i.store_id = 1;

SELECT COUNT(*) AS filter_in_on
FROM film AS f
LEFT JOIN inventory AS i ON i.film_id = f.film_id AND i.store_id = 1;
```
...
```sql practice
-- hint: `LEFT JOIN` to `inventory`, then `WHERE i.inventory_id IS NULL`, and `COUNT(*)`.
...
```sql practice
-- hint: `COUNT(i.inventory_id)`, grouped by film.
...
Show the `title` of every film that has **no copy in store 2** (it may have copies elsewhere). Order by title, and show the first rows only. (Keep the store condition in the `ON`.)

```sql practice rows=5
-- hint: `LEFT JOIN inventory ... ON i.film_id = f.film_id AND i.store_id = 2`, then keep rows where the inventory id IS NULL.
SELECT f.title
FROM film AS f
LEFT JOIN inventory AS i ON i.film_id = f.film_id AND i.store_id = 2
WHERE i.inventory_id IS NULL
ORDER BY f.title;
```
