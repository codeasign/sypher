...
```sql practice
-- hint: `LAG(length) OVER (ORDER BY film_id)`, then filter after computing.
WITH lagged AS (
...
```sql practice rows=6
-- hint: `TIMESTAMPDIFF(HOUR, LAG(rental_date) OVER (ORDER BY rental_date), rental_date)`.
SELECT rental_id, rental_date,
...
```sql practice
-- hint: Compute the change in a CTE, and keep rows where it is above 0.
WITH changes AS (
  SELECT payment_id, payment_date, amount,
         amount - LAG(amount) OVER (ORDER BY payment_date) AS change_from_previous
  FROM payment
