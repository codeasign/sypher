...
| Tables_in_sypher-mysql-DvdRental (scratch%) |
|---|
| scratch_b |
...
```text
ERROR 1051 (42S02): Unknown table 'sypher-mysql-DvdRental.no_such_table'
```
...
```text
ERROR 3730 (HY000): Cannot drop table 'team' referenced by a foreign key constraint 'fk_player_team' on table 'player'.
```
...
```text
ERROR 1146 (42S02): Table 'sypher-mysql-DvdRental.big_payments' doesn't exist
```
...
Count rows in `information_schema.tables` where `table_name = 'trash'`.
...
`DROP TABLE employee, department;` lists the child first.
