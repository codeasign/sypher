...
`UPDATE film_copy f JOIN film_category fc ... JOIN category c ... SET f.rental_rate = 0.99 WHERE c.name = 'Children'`.
...
A film has many copies, so a plain join would match it several times. Use `NOT EXISTS` with a subquery that joins `inventory` and `rental`.
...
Join `film_copy` to a derived table that counts rentals per film, and set the rate where the count is above 30. Note some films were already 4.99 before.
