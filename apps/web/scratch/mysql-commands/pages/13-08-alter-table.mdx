...
|---|---:|---|---:|---|---|
| member | 0 | PRIMARY | 1 | member_id | BTREE |
| member | 0 | unique_member_name | 1 | name | BTREE |
| member | 1 | idx_joined | 1 | joined_on | BTREE |
...
```text
ERROR 1265 (01000): Data truncated for column 'body' at row 1
```
...
`RENAME COLUMN` and `MODIFY COLUMN` (two statements), then query information_schema.columns.
...
`ALTER TABLE ... ADD CONSTRAINT weight_positive CHECK (...)`.
