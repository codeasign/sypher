...
| title | rating | length | avg_length_of_rating |
|---|---|---:|---:|
...
| title | rating | length | minutes_from_average |
|---|---|---:|---:|
...
| payment_id | amount | customer_total | percent_of_total |
|---:|---:|---:|---:|
| 1 | 2.99 | 118.68 | 2.5 |
| 2 | 0.99 | 118.68 | 0.8 |
| 3 | 5.99 | 118.68 | 5.0 |
...
| title | length | longest_film | total_films |
|---|---:|---:|---:|
| ACADEMY DINOSAUR | 86 | 185 | 1000 |
| ACE GOLDFINGER | 48 | 185 | 1000 |
| ADAPTATION HOLES | 50 | 185 | 1000 |
| AFFAIR PREJUDICE | 117 | 185 | 1000 |
| AFRICAN EGG | 130 | 185 | 1000 |
| … | … | … | … |
...
| avg_of_all_pg_films | avg_of_the_first_100_pg_films |
|---:|---:|
| 112.0 | 121.2 |
...
|---|---|---:|---:|
| ACADEMY DINOSAUR | PG | 86 | 127.5 |
| ACE GOLDFINGER | G | 48 | 98.3 |
| ADAPTATION HOLES | NC-17 | 50 | 50.0 |
| AFFAIR PREJUDICE | G | 117 | 98.3 |
| AFRICAN EGG | G | 130 | 98.3 |
| … | … | … | … |
...
`ROUND(AVG(length) OVER (PARTITION BY rating), 1)`. Careful: `WHERE` runs first, so the average is taken over only the films that survive the filter.
...
`ROUND(100 * amount / SUM(amount) OVER (PARTITION BY customer_id), 1)`.
...
Compute the window average in a CTE, and filter in the outer query.
