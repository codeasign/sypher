...
Correlate on `customer_id` inside the subquery.
...
`NOT EXISTS` with `i.film_id = f.film_id AND i.store_id = 1`.
...
`NOT EXISTS` over `film_actor` joined to `film_category` and `category`.
