...
```text
ERROR 1062 (23000): Duplicate entry '1' for key 'stock.PRIMARY'
```
...
`INSERT ... VALUES (1, 10), (9, 2) AS new ON DUPLICATE KEY UPDATE copies = stock.copies + new.copies`.
...
The second insert would clash on `comedy` and `drama`; IGNORE skips them and adds `horror`.
...
After REPLACE the note is back to its default, `new`.
