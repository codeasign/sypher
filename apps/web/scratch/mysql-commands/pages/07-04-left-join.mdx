...
`LEFT JOIN` to `inventory`, then `WHERE i.inventory_id IS NULL`, and `COUNT(*)`.
...
`COUNT(i.inventory_id)`, grouped by film.
...
`LEFT JOIN inventory ... ON i.film_id = f.film_id AND i.store_id = 2`, then keep rows where the inventory id IS NULL.
