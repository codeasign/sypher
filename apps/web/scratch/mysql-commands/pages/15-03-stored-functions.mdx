...
`RETURN p_length > 150;` gives 1 or 0.
...
`SELECT COUNT(*) FROM film WHERE is_long(length) = 1`.
...
A subquery joining `film_category` and `category`, as the RETURN value.
