...
`SELECT COUNT(*) FROM information_schema.check_constraints WHERE constraint_schema = 'bookshop'`.
...
A link table with PRIMARY KEY (book_id, category_id), then join to category.
