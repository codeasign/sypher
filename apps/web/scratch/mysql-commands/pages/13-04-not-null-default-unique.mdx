...
| email | level |
|---|---|
| asha@example.com | bronze |
| ben@example.com | bronze |
...
```text
ERROR 1062 (23000): Duplicate entry 'asha@example.com' for key 'member.email'
```
...
```text
ERROR 1062 (23000): Duplicate entry '1-gold' for key 'member_badge.one_badge_per_member'
```
...
Several NULLs are allowed in a UNIQUE column.
...
All three pairs are different, so all three are accepted.
