...
|---:|---|---|---|
| 1 | `START TRANSACTION;` |  |  |
| 2 | `UPDATE account SET balance = balance + 1`<br>`WHERE id = 1;` |  |  |
| 3 |  | `UPDATE account SET balance = balance + 5`<br>`WHERE id = 1;`<br>→ ⏳ *waited for a lock, then ran* |  |
| 4 |  |  | `SELECT COUNT(*) AS transactions_open FROM`<br>`information_schema.innodb_trx;`<br>→ **2** |
| 5 |  |  | `SELECT COUNT(*) AS transactions_waiting FROM`<br>`information_schema.innodb_trx WHERE trx_state = 'LOCK WAIT';`<br>→ **1** |
| 6 |  |  | `SELECT locked_table_name, waiting_query,`<br>`blocking_query FROM sys.innodb_lock_waits;`<br>→ locked_table_name=**account**, waiting_query=**UPDATE account SET balance = balance + 5 WHERE id = 1**, blocking_query=**NULL** |
| 7 | `ROLLBACK;` |  |  |
...
|---:|---|---|---|
| 1 | `START TRANSACTION;` |  |  |
| 2 | `UPDATE account SET balance = balance + 1`<br>`WHERE id = 2;` |  |  |
| 3 |  | `UPDATE account SET balance = balance + 1`<br>`WHERE id = 2;`<br>→ ⏳ *waited for a lock, then ran* |  |
| 4 |  |  | `SELECT COUNT(*) AS transactions_waiting FROM`<br>`information_schema.innodb_trx WHERE trx_state = 'LOCK WAIT';`<br>→ **1** |
| 5 | `ROLLBACK;` |  |  |
...
| 2 | `UPDATE account SET balance = balance + 1`<br>`WHERE id = 2;` |  |  |
| 3 |  | `UPDATE account SET balance = balance + 1`<br>`WHERE id = 2;`<br>→ ⏳ *waited for a lock, then ran* |  |
| 4 |  |  | `SELECT locked_table_name, waiting_lock_mode`<br>`FROM sys.innodb_lock_waits;`<br>→ locked_table_name=**account**, waiting_lock_mode=**X,REC_NOT_GAP** |
| 5 | `ROLLBACK;` |  |  |
...
| 5 |  |  | `SELECT COUNT(*) AS transactions_waiting FROM`<br>`information_schema.innodb_trx WHERE trx_state = 'LOCK WAIT';`<br>→ **0** |
| 6 | `ROLLBACK;` |  |  |
| 7 |  | `ROLLBACK;` |  |
