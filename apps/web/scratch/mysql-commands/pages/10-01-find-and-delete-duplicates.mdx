...
Put the `GROUP BY ... HAVING` query in a derived table and `COUNT(*)` its rows.
...
`ROW_NUMBER() OVER (PARTITION BY first_name, last_name ORDER BY actor_id DESC)` and keep numbers above 1.
...
Same self-join delete, but delete the row that has a twin with a HIGHER id.
