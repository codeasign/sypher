...
```text
ERROR 1055 (42000): Expression #2 of SELECT list is not in GROUP BY clause and contains nonaggregated column 'sypher-mysql-DvdRental.rental.rental_id' which is not functionally dependent on columns in GROUP BY clause; this is incompatible with sql_mode=only_full_group_by
```
...
*Showing the first 6 of 623 rows.*
...
| 6 | 2005-05-25 08:43:32 | 2005-08-23 06:41:32 |
| … | … | … |
...
`ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY amount DESC, payment_id)`, keep `rn = 1`.
...
Rank each customer's rentals oldest-first, join inventory and film for the title.
...
`MIN`, `MAX` and `DATEDIFF(MAX(...), MIN(...))` with a GROUP BY.
