...
| connected_as | films_it_can_read |
|---|---:|
| report_reader@% | 1000 |
...
```text
ERROR 1142 (42000): INSERT command denied to user 'report_reader'@'localhost' for table 'category'
```
...
```text
ERROR 1142 (42000): SELECT command denied to user 'report_reader'@'localhost' for table 'user'
```
...
```text
ERROR 1143 (42000): SELECT command denied to user 'limited'@'localhost' for column 'email' in table 'customer'
```
...
`GRANT SELECT ON `sypher-mysql-DvdRental`.category TO 'clerk'@'%'`, then `SHOW GRANTS FOR ...`.
...
GRANT SELECT, UPDATE ... then REVOKE UPDATE ... then SHOW GRANTS.
...
`CREATE ROLE`, `GRANT SELECT ... TO 'viewer'`, `GRANT 'viewer' TO 'guest'@'%'`.
