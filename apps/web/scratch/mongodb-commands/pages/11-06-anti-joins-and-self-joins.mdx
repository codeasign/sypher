...
```text
[
  { _id: { a: 27, b: 60 }, films: 7 },
  { _id: { a: 15, b: 198 }, films: 6 },
  { _id: { a: 21, b: 108 }, films: 6 }
]
```
...
```text
[
  { _id: { id: 4, name: 'JENNIFER DAVIS' }, together: 4 },
  { _id: { id: 20, name: 'LUCILLE TRACY' }, together: 3 },
  { _id: { id: 64, name: 'RAY JOHANSSON' }, together: 3 }
]
```
...
`$not` with `$elemMatch` on `returnDate: null`.
...
Match films with actor 2, unwind, exclude 2, group by id, count.
