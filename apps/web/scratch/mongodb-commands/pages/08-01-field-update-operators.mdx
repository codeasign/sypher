...
{ qty: 4, restocked: true, title: 'ink' }
```
...
```text
MongoServerError: Updating the path 'qty' would create a conflict at 'qty'
```
...
`$rename` and `$unset` in one update.
...
One update per reading, with both operators.
