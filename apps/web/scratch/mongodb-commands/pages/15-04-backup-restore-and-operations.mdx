...
```text
done dumping `sypher-mongodb-DvdRental.films` (1000 documents)
```
...
```text
1000 document(s) restored successfully
```
...
```text
{ op: 'query', ns: 'lab_ops.t', planSummary: 'COLLSCAN' }
```
...
`$out` then `deleteOne`, then `$merge` back.
...
Read `system.profile` filtered by `ns`.
