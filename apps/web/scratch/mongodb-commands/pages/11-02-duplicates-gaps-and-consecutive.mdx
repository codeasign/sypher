...
```text
[
  { _id: 'a@x.com', orders: 3, ids: [ 1, 3, 7 ] },
  { _id: 'b@x.com', orders: 2, ids: [ 2, 5 ] }
]
```
...
```text
[
  { _id: 'a@x.com', remove: 1 },
  { _id: 'b@x.com', remove: 2 },
  { _id: 'a@x.com', remove: 3 }
]
```
...
```text
[ { missingFrom: 4, missingTo: 4 }, { missingFrom: 8, missingTo: 9 } ]
```
...
```text
[
  { _id: { runId: 2, status: 'fail' }, length: 4 },
  { _id: { runId: 1, status: 'ok' }, length: 2 }
]
```
...
```text
[ { from: 3, to: 3 }, { from: 5, to: 7 } ]
```

<details>
<summary>Hint</summary>

`$shift` by -1, then compare.
...
Compare `$shift` by -1 and -2 with the current value; then `$group` the values.
