...
```sql practice
-- hint: Join `category` to itself on `length(c1.name) = length(c2.name)` and `c1.category_id < c2.category_id`.
...
```sql practice
-- hint: Put `address` next to each `customer` copy and compare `city_id`.
