...
```sql practice
-- hint: Count the distinct `island_key` values.
WITH days AS (
  SELECT DISTINCT lower(rental_period)::date AS day FROM rental
),
keyed AS (
...
```sql practice
-- hint: Build the islands, then `ORDER BY days_in_a_row DESC, first_day LIMIT 1`.
WITH days AS (
  SELECT DISTINCT lower(rental_period)::date AS day FROM rental
),
keyed AS (
...
```sql practice
-- hint: Per-customer islands, keep groups with COUNT(*) >= 5, count distinct customers.
WITH cust_days AS (
  SELECT DISTINCT customer_id, lower(rental_period)::date AS day FROM rental
),
keyed AS (
