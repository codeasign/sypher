...
-- hint: `constraint_type = 'CHECK'` also lists system NOT NULL checks; filter `constraint_name NOT LIKE '%not_null'`.
CREATE SCHEMA shop2;
...
```sql practice destructive
-- hint: A link table with `PRIMARY KEY (book_id, category_id)`.
CREATE SCHEMA shop4;
