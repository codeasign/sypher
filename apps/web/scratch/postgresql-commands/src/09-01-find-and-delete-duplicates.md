...
```sql practice
-- hint: Count the groups from Step 1 in a subquery.
...
```sql practice
-- hint: `ROW_NUMBER() OVER (PARTITION BY first_name, last_name ORDER BY actor_id DESC)`, keep `rn > 1`.
SELECT actor_id
FROM (
...
-- hint: `DELETE ... USING` with `a.actor_id < b.actor_id`.
