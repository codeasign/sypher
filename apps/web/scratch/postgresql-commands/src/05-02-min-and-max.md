...
The first and last payment in the database:
...
```sql run
SELECT MAX(length) AS longest_g
FROM film
WHERE rating = 'G';
```
...
PostgreSQL rejects it, because `MAX` gives one number for the whole table while `title` has one value per row. To get the row, ask for it in order:
...
Or filter for the maximum with a subquery (Module 7):
...
```sql run
SELECT MAX(original_language_id) AS highest
FROM film;
```

### MIN and MAX do not work on some types

They work on numbers, text, dates and enums, but not on booleans or JSON. For booleans use `bool_and` and `bool_or`.
...
```sql practice
-- hint: `MIN` and `MAX` on `payment_date`.
SELECT MIN(payment_date) AS first_payment, MAX(payment_date) AS last_payment
FROM payment;
```
...
Show the `title` and `length` of **every** film that is as long as the longest film. Order by title. Show the first rows.

```sql practice
-- hint: `WHERE length = (SELECT MAX(length) FROM film)`.
SELECT title, length
FROM film
WHERE length = (SELECT MAX(length) FROM film)
ORDER BY title;
