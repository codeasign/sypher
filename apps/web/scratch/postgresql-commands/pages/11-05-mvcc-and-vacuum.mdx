...
`SELECT id, ctid::text FROM mover ORDER BY id` after the UPDATE.
...
`CREATE TABLE sizes AS SELECT pg_relation_size('churn') AS s` after each step, then compare.
