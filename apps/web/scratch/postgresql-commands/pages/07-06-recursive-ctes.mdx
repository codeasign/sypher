...
Anchor `SELECT 1`, recursive part `SELECT month_number + 1 ... WHERE month_number < 12`.
...
Same shape as the example, `LEFT JOIN` rental on `lower(rental_period)::date`.
...
Carry two columns: `exponent + 1` and `value * 2`.
