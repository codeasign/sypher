...
| last_name | title |
|---|---|
| HUNTER | BLANKET BEVERLY |
| COLLAZO | FREAKY POCUS |
| MURRELL | GRADUATE LORD |
...
Join `film`, `film_category` and `category`; filter `c.name = 'Horror'`.
...
customer -> address -> city -> country, then GROUP BY and HAVING.
...
Six tables, one chain: payment.rental_id = rental.rental_id, and so on.
