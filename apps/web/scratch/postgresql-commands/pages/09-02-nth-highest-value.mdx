...
`SELECT DISTINCT length ... ORDER BY length DESC LIMIT 1 OFFSET 2`, wrapped so the column is named.
...
`DENSE_RANK() OVER (ORDER BY length DESC)` in a subquery, keep `rnk = 2`.
...
Total per customer, `DENSE_RANK() OVER (ORDER BY total DESC)`, keep rank 2.
