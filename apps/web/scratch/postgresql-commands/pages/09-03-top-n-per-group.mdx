...
| with_row_number | with_dense_rank |
|---:|---:|
| 48 | 64 |
...
*Showing the first 6 of 48 rows.*
...
`ROW_NUMBER() OVER (PARTITION BY rating ORDER BY length, title)`, keep 1. Or `DISTINCT ON`.
...
Count rentals per film and category, `ROW_NUMBER() OVER (PARTITION BY category ORDER BY rentals DESC, title)`.
...
`ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY amount DESC, payment_id)`, keep `rn <= 3`.
