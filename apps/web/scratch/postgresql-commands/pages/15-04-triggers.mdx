...
| trigger | on_table | definition |
|---|---|---|
| last_updated | category | CREATE TRIGGER last_updated BEFORE UPDATE ON public.category FOR EACH ROW EXECUTE FUNCTION last_updated() |
...
CONTEXT:  PL/pgSQL function check_price() line 4 at RAISE
```
...
`INSERT INTO item_archive VALUES (OLD.item_id, OLD.name)`, `RETURN OLD`.
...
`IF NEW.price < OLD.price / 2 THEN RAISE EXCEPTION ...`.
