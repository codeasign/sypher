...
| about_to_change | average_before |
|---:|---:|
| 178 | 2.866 |
...
DETAIL:  Failing row contains (1, null, A Epic Drama of a Feminist And a Mad Scientist who must Battle a..., 2006, 1, null, 6, 0.99, 86, 20.99, PG, (current time), {"Deleted Scenes","Behind the Scenes"}, 'battl':13 'canadian':18 'drama':3 'epic':2 'feminist':6 'mad':9..., virtual, 59.40).
```
...
Create the copy with `CREATE TABLE film_copy2 AS SELECT * FROM film`, then `UPDATE ... RETURNING`.
...
`UPDATE ... SET replacement_cost = replacement_cost + 2 WHERE length > 150`, then `MAX`.
...
One `UPDATE` with `CASE`, then GROUP BY rental_rate.
