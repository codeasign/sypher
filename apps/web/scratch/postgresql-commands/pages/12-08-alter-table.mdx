...
| conname | contype |
|---|---|
| club_member_joined_not_null | n |
| club_member_member_id_not_null | n |
| club_member_name_not_null | n |
| club_member_pkey | p |
| name_unique | u |
...
`RENAME COLUMN`, then `ALTER COLUMN ... TYPE varchar(100)`.
...
`ALTER TABLE ... ADD CONSTRAINT weight_positive CHECK (weight_kg > 0)`, then look in `pg_constraint`.
