...
Gather
  ...
  ->  Parallel Seq Scan on big
        Filter: (date_trunc('day'::text, ts) = '2020-03-01 00:00:00'::timestamp without time zone)
```
...
```text
Index Scan using idx_big_ts on big
  Index Cond: ((ts >= '2020-03-01 00:00:00'::timestamp without time zone) AND (ts < '2020-03-02 00:00:00'::timestamp without time zone))
```
...
```text
Index Scan using big_pkey on big
  Index Cond: (id = 500)
```
...
```text
Index Scan using big_pkey on big
  Index Cond: (id = ANY ('{500,700}'::integer[]))
```
...
Create the table first (as in Set up), then use the half-open range.
...
```text
Index Scan using idx_cx_last on customer_x
...
`CREATE INDEX ON customer_x (last_name)`, then EXPLAIN.
...
```text
Seq Scan on film_x
  Filter: (rental_duration > 2)
```

<details>
<summary>Hint</summary>

Nearly every film has a `rental_duration` above 2.
