...
```text
ERROR:  duplicate key value violates unique constraint "stock_pkey"
...
`ON CONFLICT DO NOTHING` needs no column list when there is one unique key.
...
`MERGE INTO stock3 t USING (VALUES (1, 5), (3, 7)) AS s(film_id, copies) ON ... WHEN MATCHED ... WHEN NOT MATCHED ...`.
