...
Join film to film_category and category, AVG(length) OVER (PARTITION BY category) in a CTE.
...
`AVG(amount) OVER (PARTITION BY customer_id)` in a CTE, then `amount > avg_amount`.
