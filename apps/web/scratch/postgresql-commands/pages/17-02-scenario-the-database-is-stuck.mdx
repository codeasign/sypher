...
| Step | Session A | Session B | Session R (admin) |
|---:|---|---|---|
| 1 | `BEGIN` |  |  |
...
| 4 |  |  | `SELECT COUNT(*) AS waiting_on_a_lock FROM`<br>`pg_stat_activity WHERE wait_event_type = 'Lock'`<br>→ **1** |
| 5 |  |  | `SELECT COUNT(*) AS idle_in_transaction FROM`<br>`pg_stat_activity WHERE state = 'idle in transaction'`<br>→ **1** |
| 6 |  |  | `SELECT COUNT(*) AS ungranted_locks FROM`<br>`pg_locks WHERE NOT granted`<br>→ **1** |
| 7 |  |  | `SELECT COUNT(*) AS blocked_sessions, MAX(cardinality(pg_blocking_pids(pid))) AS`<br>`blockers_each FROM pg_stat_activity WHERE cardinality(pg_blocking_pids(pid)) > 0`<br>→ blocked_sessions=**1**, blockers_each=**1** |
| 8 |  |  | `SELECT COUNT(*) AS terminated FROM (SELECT pg_terminate_backend(b) FROM`<br>`(SELECT DISTINCT unnest(pg_blocking_pids(pid)) AS b FROM pg_stat_activity) x) y`<br>→ **1** |
...
|---:|---|---|---|
| 1 | `BEGIN` |  |  |
| 2 | `UPDATE wallet SET balance = balance + 1`<br>`WHERE id = 2` |  |  |
| 3 |  | `UPDATE wallet SET balance = balance + 1`<br>`WHERE id = 2`<br>→ ⏳ *waited for a lock, then ran* |  |
| 4 |  |  | `SELECT COUNT(*) AS waiting FROM`<br>`pg_stat_activity WHERE wait_event_type = 'Lock'`<br>→ **1** |
| 5 | `ROLLBACK` |  |  |
...
| 5 |  |  | `SELECT COUNT(*) AS waiting FROM`<br>`pg_stat_activity WHERE wait_event_type = 'Lock'`<br>→ **0** |
| 6 | `COMMIT` |  |  |
| 7 |  | `COMMIT` |  |
