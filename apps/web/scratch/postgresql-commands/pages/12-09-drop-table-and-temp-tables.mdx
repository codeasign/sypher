...
```text
ERROR:  cannot drop table team2 because other objects depend on it
DETAIL:  constraint player2_team_id_fkey on table player2 depends on table team2
```
...
```text
ERROR:  relation "long_films" does not exist
LINE 1: SELECT COUNT(*) FROM long_films;
                             ^
```
