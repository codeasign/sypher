...
| policyname | cmd | qual |
|---|---|---|
| own_team | ALL | (team = split_part((CURRENT_USER)::text, '_'::text, 1)) |
...
```text
ERROR:  new row violates row-level security policy for table "ticket"
```
...
`ALTER TABLE ... ENABLE ROW LEVEL SECURITY`, then read `relrowsecurity`.
...
FORCE makes the owner obey the policy.
