...
```text
NOTICE:  customer 1 paid 118.68 in total
```
...
BEGIN, DECLARE, FETCH 5, CLOSE, COMMIT: the last result set is the fetch.
...
The last result set is the result of `FETCH PRIOR`.
...
Two UPDATE statements, then COUNT(*) FILTER (WHERE done).
