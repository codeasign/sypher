...
This whole course uses one sample database, **DVD Rental**, the records of a DVD-rental company. It has 15 main tables (the `payment` table is stored in several monthly pieces, called *partitions*, which you will meet later). Here are the ones we will use most:
