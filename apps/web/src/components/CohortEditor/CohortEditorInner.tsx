...
export default function CohortEditorInner({ cohort, onSaved, onCancel, onBack }: CohortEditorProps): React.JSX.Element {
  ...
  const [coverImageUrl, setCoverImageUrl] = useState(cohort?.coverImageUrl ?? '');
  const [startDate, setStartDate] = useState(cohort?.startDate ?? '');
  const [durationWeeks, setDurationWeeks] = useState(cohort?.durationWeeks?.toString() ?? '');
  const [seatsTotal, setSeatsTotal] = useState(cohort?.seatsTotal?.toString() ?? '');
  const [priceLabel, setPriceLabel] = useState(cohort?.priceLabel ?? '');
  ...
  async function persist(nextStatus?: 'draft' | 'live' | 'closed'): Promise<void> {
    ...
    try {
      if (!isEditing) {
        const { error: createError, cohort: created } = await createCohort(currentFields());
        if (createError || !created) {
          setError(createError ?? 'Failed to create cohort.');
          return;
        }
        if (nextStatus) {
          const { error: statusError } = await setCohortStatus(created.id, nextStatus);
          if (statusError) {
        ...
        const { error: updateError } = await updateCohort(cohort!.id, currentFields());
        if (updateError) {
          setError(updateError);
          return;
        }
        if (nextStatus && nextStatus !== cohort!.status) {
          const { error: statusError } = await setCohortStatus(cohort!.id, nextStatus);
          if (statusError) {
            setError(statusError);
            return;
          }
            ...
            <div className={styles.mdxWrapper}>
              <MDXEditor
                ...
                plugins={[
                  ...
                  markdownShortcutPlugin(),
                  hardLineBreakPlugin(),
                  // Same crash fix as apps/app's ModuleEditorInner.tsx:
                  // round-tripping real content with JSX tags, frontmatter
                  // delimiters, or `---` dividers throws
                  // UnrecognizedMarkdownConstructError without these three.
                  jsxPlugin({
