...
const CourseModuleArticle = dynamic(
    () => import("@/components/CourseModulePage/CourseModuleArticle"),
    {
        ssr: false,
...
function ModulesPane({
    ...
    const loadDetail = useCallback(
        (mod: BookmarkedModule): Promise<ModuleDetail> => {
            ...
            const request = apiFetch(
                ...
                .then(async (res) => {
                    if (!res.ok)
                        throw new Error(
                            `Could not load lesson (${res.status})`,
                        );
                ...
                {groups.map((group) => (
                    <div key={group.courseId} className={styles.menuGroup}>
                        <p className={styles.menuGroupTitle}>
                            ...
                            <span className={styles.menuGroupName}>
                                {group.courseName}
                            </span>
                        </p>
                ...
                {!selected ? (
                    ...
                    <>
                        ...
                        >
                            ...
                            <span className={styles.crumbSlug}>
                                {selected.slug}
                            </span>
                        </nav>

                        {loading && (
                            <p className={styles.emptyText}>Loading lesson…</p>
                        )}
